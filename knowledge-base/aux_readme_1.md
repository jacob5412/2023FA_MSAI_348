...
- `knowledge_base.py`: Contains the `KnowledgeBase` class and is where you will be writing code
...
To get you started, the stubbed-out code for the `KnowledgeBase.kb_assert` and `KnowledgeBase.kb_ask` methods are in `knowledge_base.py`. These methods are called by the tests in `main.py`.
