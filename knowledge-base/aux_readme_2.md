...
- `knowledge_base.py`: Contains the `KnowledgeBase` and `InferenceEngine` classes and is where you will be writing code.
...
- Asserted facts and rules that DO NOT have support can be retracted. But note,
  - Facts and rules that are asserted without any supporting evidence can only be retracted when you first attempt to do so. 
  - However, when trying to retract supported asserted facts, it's advisable not to do so after removing just one of the supporting pieces of evidence.
