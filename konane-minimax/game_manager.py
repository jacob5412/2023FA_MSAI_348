...
class GameManager:
    ...
    def GetTurn(self):
        if self.state == AWAITING_INITIAL_X or self.state == X_TURN:
            return self.p1.symbol
        if self.state == AWAITING_INITIAL_O or self.state == O_TURN:
            return self.p2.symbol
        if self.state == X_VICTORY or self.state == O_VICTORY:
            return ""
    ...
    def _takeTurn(self, move_pair=None):
        ...
        if len(game_rules.getLegalMoves(self.board, self.GetTurn())) < 1:
            if self.state == X_TURN:
                self.state = O_VICTORY
            if self.state == O_TURN:
    ...
    def _handleInitialX(self, playerBoard, board, move_pair):
        move = (
            ...
            else self.p1.selectInitialX(playerBoard)
        )
    ...
    def _handleInitialO(self, playerBoard, board, move_pair):
        move = (
            move_pair[0]
            if move_pair is not None
            else self.p2.selectInitialO(playerBoard)
        )
            ...
            for each in f:
                ...
                if len(each) > 1:
                    ...
                    if len(parts) < 4:
                        ...
                        move = (
                            (int(parts[0][2:]), int(parts[1][:-1])),
                            (int(parts[2][2:]), int(parts[3][1:-3])),
                        )
                    print(move)
