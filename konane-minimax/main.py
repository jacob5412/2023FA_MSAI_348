...
class Display(object):
    ...
    def __init__(self, stdscr, manager):
        ...
        if isinstance(self._gm.p1, player.HumanPlayer):
            self._p1state = self.PlayerInputState()
        if isinstance(self._gm.p2, player.HumanPlayer):
            self._p2state = self.PlayerInputState()

        inbounds = self._dimensions > (self._gm.rows + 3, self._gm.cols + 3)
        # ASSERTIONS ARE CONTRACTS WITHOUT BLAME.
        # We do this so that the screen isn't too small. There will still be problems if people resize.
        assert inbounds, "Screen too small for board: {} vs {}".format(
            (self._gm.rows, self._gm.cols), self._dimensions
        )
    ...
    def SetDimensions(self):
        ...
        self._start = (
            width // 2 - self._gm.cols // 2,
            height // 2 - self._gm.rows // 2,
        )
    ...
    def DrawStatus(self, stdscr):
        ...
        text = " CTRL-c to quit"
        if self._gm.GetWinner():
            text += " | {} wins!".format(self._gm.GetWinner())
        else:
            text += " | Turn {0:{width}} ({1}, {2}) ".format(
                self._gm.turn_number, symbol, player, width=3
            )
        height, width = self._dimensions
    ...
    def ProcessInput(self, stdscr):
        ...
        if self._last == curses.KEY_DOWN:
            self.MoveCursor(0, 1)
        elif self._last == curses.KEY_UP:
            self.MoveCursor(0, -1)
        elif self._last == curses.KEY_RIGHT:
            self.MoveCursor(1, 0)
        elif self._last == curses.KEY_LEFT:
            self.MoveCursor(-1, 0)
        elif self._last == 32:
            self.TakeTurn()
    ...
    def TakeTurn(self):
        if self._gm.GetTurn() == "x" and self._p1state:
            if self._p1state.selected:
                ...
                self._p1state.Select(self._cursor)
            ...
            if self._p2state.selected:
                ...
                self._p2state.Select(self._cursor)
        else:
            self._gm._takeTurn()
    ...
    class PlayerInputState(object):
        """Container class for input state. Just keep adding layers of
        abstraction; it's the computer science way."""
