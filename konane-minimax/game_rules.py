...
def moveLength(move):
    return (
        abs(move[0][0] - move[1][0])
        if verticalMove(move)
        else abs(move[0][1] - move[1][1])
    )
...
def isLegalJump(board, player, other, jump):
    return (
        ...
        and pieceAt(board, midPoint(jump)) == other
        and pieceAt(board, jump[1]) == " "
    )
...
def getNeighbors(board, point):
    ...
    return set(
        filter(
            lambda pt: onBoard(rows, cols, pt),
            [
                (point[0] - 1, point[1]),
                (point[0] + 1, point[1]),
                (point[0], point[1] - 1),
...
def getFirstMovesForX(board):
    return set(
        filter(
            lambda pt: pieceAt(board, pt) == "x",
            getCorners(board).union(getMiddles(board)),
        )
    )
...
def delinearizeBoard(rawBoard, rows, cols):
    ...
    assert len(board) == rows, print(
        "Problem parsing board! Expected {} rows, got {}.".format(rows, len(board))
    )
    return board
